...
func serialize_movement(move_dict:Dictionary) -> PoolByteArray:
	...
	if should_send_move:
		...
		if has_look_changed:
			put_u16(float_to_unsigned_k_bit_int(
				move_dict['yaw_0'], 0.0, 360.0, 16))
			put_u8(float_to_unsigned_k_bit_int(
				move_dict['pitch_0'], -90.0, 90.0, 8))
			
			put_u16(float_to_unsigned_k_bit_int(
				move_dict['yaw_1'], 0.0, 360.0, 16))
			put_u8(float_to_unsigned_k_bit_int(
				move_dict['pitch_1'], -90.0, 90.0, 8))
...
func deserialize_movement(instruc:PoolByteArray, move_dict:Dictionary):
	...
	if has_look_changed:
		move_dict['yaw_0'] = float_from_unsigned_k_bit_int(
			get_u16(), 0.0, 360.0, 2)
		move_dict['pitch_0'] = float_from_unsigned_k_bit_int(
			get_u8(), -90.0, 90.0, 1)
		
		move_dict['yaw_1'] = float_from_unsigned_k_bit_int(
			get_u16(), 0.0, 360.0, 2)
		move_dict['pitch_1'] = float_from_unsigned_k_bit_int(
			get_u8(), -90.0, 90.0, 1)
...
static func float_to_unsigned_k_bit_int(f:float, f_min:float, f_max:float, 
k:int) -> int:
	assert(
		f >= f_min and f <= f_max, 
	...
	assert(
		k > 0 and k <= 32,
		"k must be between [1..32] bits")
	k = int(clamp(k, 1, 32))
	
	var k_bit_int_max = (1 << k) - 1
	var lerp_frac = (f - f_min) / (f_max - f_min)
	
	return int(lerp(0, k_bit_int_max, lerp_frac))

static func float_from_unsigned_k_bit_int(i:int, f_min:float, f_max:float, 
k:int) -> float:
	assert(
		k > 0 and k <= 32,
		"k must be between [1..32] bits")
	...
	var k_bit_int_max = (1 << k) - 1
	assert(
		i >= 0 and i <= k_bit_int_max, 
		"given int must fall between 0 and %d" % k_bit_int_max)
	i = int(clamp(i, 0, k_bit_int_max))
	
	var lerp_frac = float(i) / k_bit_int_max
